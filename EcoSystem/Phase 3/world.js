function World(width, height){
    ...
    this.movers = [];
    this.newMover(10, ctxArr,this.dimensions.x,this.dimensions.y);

     this.particles = [];
     this.psystem = [];
     this.newPSystem(2, ctxArr,this.dimensions.x,this.dimensions.y);

      this.birds = [];
      this.newBird(200, ctxArr,this.dimensions.x,this.dimensions.y);
...
World.prototype.run = function(){
  ...
  for(var i = 0;i<this.psystem.length;i++){
   this.psystem[i].run();
 }
...
//snakez code for them to work and whatever
...
//moverz code for them to work and whatever
World.prototype.newMover = function(numMovers,ctxArr,worldWidth,worldHeight){
for(var i = 0; i<numMovers;i++){
  ...
  this.movers.push(new Mover(x, y, dx, dy, rad, clr, numOrbiters, ctxArr,worldWidth,worldHeight));
  }
}

World.prototype.newPSystem = function(numSys,ctxArr,worldWidth,worldHeight){
  for(var i = 0; i<numSys;i++){
  x = Math.random()*(worldWidth-2)-worldWidth/2;
  y = Math.random()*(worldHeight-2)-worldHeight/2;
  this.psystem.push(new ParticleSys(x,y,ctxArr,worldWidth,worldHeight));
  }
}

World.prototype.newBird = function(numBirds,ctxArr,worldWidth,worldHeight){
  for(var i=0;i<numBirds;i++){
  this.birds.push(new Bird(new JSVector(Math.random()*worldWidth-worldHeight/2, Math.random()*worldHeight-worldHeight/2/2), ctxArr));
  }
}
